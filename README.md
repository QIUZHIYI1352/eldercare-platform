...
>
> **在平台里两步搞定（推荐）**：`python run.py` → 登录后进「实时监控」页，
> 点 **「用手机当摄像头」**，页面直接给出二维码；手机扫码 → 点「允许」→
> 视频源会自动填好，点「开始监控」即可。这台手机会自动登记成设备
> **「手机摄像头」**，以后在下拉里直接选，不必再扫码。
>
> 不想开平台时，也可以单独跑：

```bash
python phone_cam.py     # 打印地址与终端二维码，手机浏览器打开即可
```
